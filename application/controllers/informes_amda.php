...
class Informes_amda extends CI_Controller {
        ...
        public function Index($periodo=NULL) {
            ...
            if($list_informes->num_rows() > 0)
             ...
             $data['periodos']=  $this->Periodo();   
             if($periodo == NULL){
             $mes_actual=date('m');
             }  else {
                 $mes_actual=$periodo;
             }
            $data['informes_mes']=  $this->informes_model->informes_mes($mes_actual);        
...
$informe=$root->appendChild($informe);
                      $datos_informe=$this->xml_model->datos_informe($key);
 /********************FOREAH*************/
...
//	$descripcion_modificacion = $modificatorio->appendChild($descripcion_modificacion);
    ...
    $persona_aviso =$aviso->appendChild($persona_aviso);
            ...
            // $tipo_persona = $persona_aviso->appendChild($tipo_persona);  
 /*****************************************************************************************/           
      $datos_aviso=  $this->xml_model->datos_aviso($key);
   ...
   $count_beneficiario = $this->xml_model->count_beneficiario($key);
 ...
 //recuperamos las operaciones realizadas para el aviso
         $operaciones = $this->xml_model->total_operaciones($key);
...
$query_referencia= "amda_aviso";//
//}//fin foreach
//echo 'Se creo el XML';
$urlxml="./public/xml/miarchivoxml3.xml";
        $this->descargar_xml($urlxml,$query_referencia);
             
   }
           /*/////**/
             
           public function CreateXMl($id_aviso)
                   {
...
$urlxml="./public/xml/miarchivoxml3.xml";
    ...
    public function Periodo()
        ...
        $now=  date('Ym');
        $periodos=array();
                //llena array de periodos disponibles
      /*  for($periodo="201309";$periodo<=$now;$periodo++)
            {
